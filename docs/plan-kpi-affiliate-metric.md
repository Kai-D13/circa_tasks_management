...
6. **Status vocabulary (cập nhật H1.1 — 27/07):** nguồn xuất hiện raw status MỚI `WAIT_TO_DELIVER` (run 27/07 `success_with_notes`, unknown → health chặn READY, mọi màn tiền ẩn số — fail-closed hoạt động đúng thiết kế). **Phân loại đã chốt: `'waiting'`** (đơn chờ giao — KHÔNG tính GMV, không đổi rule DELIVERED-only). Known-set = 8 raw status. Quy trình chuẩn khi nguồn thêm status mới: health tự fail-closed → phân loại vào `STATUS_NORM` (stakeholder ack qua audit) → deploy → cron full-snapshot tự re-upsert `status_norm` các đơn cũ → run kế `unknown_statuses=[]` → READY tự hồi.
7. **Health fail-closed trên UI (P3-I r1.1, 24/07):** mọi màn hiển thị số Affiliate (overview super + card SM/QLCH) gọi `getAffiliateSyncHealth` TRƯỚC; chỉ khi READY mới gọi `rpc_aggregate_affiliate_gmv`. Không READY (run running/failed · stale >180' · rejected>0 · unmatched/unknown · note · canary completed_time · lỗi lookup) → không aggregate, hiện `—` + lý do + lần sync thành công gần nhất — không bao giờ 0 giả.
