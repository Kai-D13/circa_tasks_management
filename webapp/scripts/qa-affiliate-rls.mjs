...
//   node scripts/qa-affiliate-rls.mjs check <email> <password>   # login + đọc affiliate_orders theo RLS
...
//   staff CÙNG dept OPS (role staff)→ chỉ theo store mình (dept không mở quyền)
...
async function status() {
  const o = await svc.from('affiliate_orders').select('*', { count: 'exact', head: true })
  const f = await svc.from('affiliate_orders').select('order_id', { count: 'exact', head: true }).in('order_id', FIX_IDS)
  const r = await svc.from('affiliate_sync_runs').select('*', { count: 'exact', head: true }).eq('status', 'running')
  const g = await svc.from('affiliate_department_access').select('department_id')
...
async function fixtureUp() {
  const stores = await svc.from('stores').select('id, code').in('code', ['POS0059', 'POS0009', 'POS0088'])
  const by = Object.fromEntries(stores.data.map((s) => [s.code, s.id]))
  ...
  const ins = await svc.from('affiliate_orders').insert(rows)
  console.log(ins.error ? 'FAIL: ' + ins.error.message : 'OK: đã tạo 4 đơn QA (' + FIX_IDS.join(', ') + ')')
}

async function check() {
  if (!email || !password) { console.log('Cách dùng: node scripts/qa-affiliate-rls.mjs check <email> <password>'); process.exit(1) }
  const anon = createClient(URL, anonKey, { auth: { persistSession: false } })
  const login = await anon.auth.signInWithPassword({ email, password })
  if (login.error) { console.log('LOGIN FAIL:', login.error.message); process.exit(1) }
  const c = createClient(URL, anonKey, {
    global: { headers: { Authorization: 'Bearer ' + login.data.session.access_token } },
    auth: { persistSession: false },
  })
  const prof = await svc.from('users').select('role, department_id, store_id, stores!users_store_id_fkey(code, store_type)').eq('email', email).single()
  ...
  const rows = await c.from('affiliate_orders').select('order_code').in('order_id', FIX_IDS).order('order_id')
  console.log('THẤY', rows.data?.length ?? 0, 'đơn:', (rows.data ?? []).map((x) => x.order_code).join(', ') || '(không đơn nào)')
  const rpc = await c.rpc('rpc_start_affiliate_sync')
  console.log('gọi rpc_start:', rpc.error ? 'BỊ CHẶN ✓ (' + rpc.error.message.slice(0, 45) + ')' : 'FAIL — GỌI ĐƯỢC (' + rpc.data + ')')
}

async function grantOps() {
  const pre = await svc.from('affiliate_department_access').select('department_id').eq('department_id', OPS_DEPT)
  ...
  const ins = await svc.from('affiliate_department_access').insert({ department_id: OPS_DEPT })
  console.log(ins.error ? 'FAIL: ' + ins.error.message : 'OK: đã cấp quyền dept OPS (pre-state: chưa có → revoke-ops sẽ xóa lại).')
}

async function revokeOps() {
  if (!fs.existsSync(MARKER)) { console.log('Không có pre-state (.qa-ops-grant.json) — chạy grant-ops trước, không tự ý xóa.'); return }
  ...
  if (pre.existedBefore) {
    ...
    const del = await svc.from('affiliate_department_access').delete().eq('department_id', OPS_DEPT).select()
    console.log(del.error ? 'FAIL: ' + del.error.message : 'OK: đã khôi phục pre-state (xóa grant OPS, deleted=' + del.data.length + ')')
  }
  fs.unlinkSync(MARKER)
}

async function fixtureDown() {
  const del = await svc.from('affiliate_orders').delete().in('order_id', FIX_IDS).select()
  console.log(del.error ? 'FAIL: ' + del.error.message : 'OK: đã xóa ' + del.data.length + ' đơn QA theo exact IDs')
  const o = await svc.from('affiliate_orders').select('*', { count: 'exact', head: true })
  const r = await svc.from('affiliate_sync_runs').select('*', { count: 'exact', head: true }).eq('status', 'running')
...
if (!cmds[cmd]) { console.log('Lệnh: status | fixture-up | check <email> <password> | grant-ops | revoke-ops | fixture-down'); process.exit(1) }
await cmds[cmd]()
