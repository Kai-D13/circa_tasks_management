...
export function TaskStatusSelector({ taskId, currentStatus, userRole }: Props) {
  ...
  const visibleStatuses = userRole === 'admin'
    ? allStatuses
    : allStatuses.filter((s) => s !== 'overdue')
  ...
  return (
    <div className="space-y-2">
      <Select value={currentStatus} onValueChange={handleChange} disabled={pending}>
        <SelectTrigger className="h-7 w-40 text-xs">
          <SelectValue>{STATUS_LABEL[currentStatus]}</SelectValue>
        </SelectTrigger>
        <SelectContent>
      ...
      {pendingStatus && (
        <div className="rounded-md border bg-muted/40 p-3 space-y-2 text-sm">
          <p className="text-xs text-muted-foreground">Ghi chú tiến độ (không bắt buộc)</p>
          <Textarea
            value={note}
          ...
          <div className="flex gap-2">
            <Button size="sm" className="text-xs h-7" disabled={pending}
