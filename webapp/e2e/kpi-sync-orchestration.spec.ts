  ...
  for (const pos of ['POS0001', 'POS0002']) {
    for (let t = Date.parse('2026-07-01T00:00:00Z'); t <= Date.parse('2026-07-23T00:00:00Z'); t += DAY_MS) {
      ...
      rows.push({
        ...
        offline_revenue_null_count: 0, offline_order_null_count: 0,
      })
    }
  }
  return rows
...
test.describe('kpi sync orchestration @desktop', () => {
  ...
  test('BQ-V2 GUARD: trùng key (pos, ngày) trong cùng lần pull → preserved, KHÔNG replace', async () => {
    const { deps, calls } = mkDeps(CFG(), {
      bq: async () => [
        { pos_code: 'POS0001', date: '2026-07-02', gmv: 300, source_row_count: 1, order_count: 0, rev_without_order: 0, order_without_rev: 0, negative_order: 0, non_integer_order: 0, revenue_with_zero_order: 0 , offline_revenue_null_count: 0, offline_order_null_count: 0 },
        { pos_code: 'POS0001', date: '2026-07-02', gmv: 999, source_row_count: 1, order_count: 0, rev_without_order: 0, order_without_rev: 0, negative_order: 0, non_integer_order: 0, revenue_with_zero_order: 0 , offline_revenue_null_count: 0, offline_order_null_count: 0 },
      ],
    })
    const r = await syncCampaignWithDeps('camp-1', deps)
    expect(r.status).toBe('snapshot_preserved')
