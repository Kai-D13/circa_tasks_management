...
export default async function DashboardLayout({
  ...
  // Site model (F5b): a dual-access account that hasn't chosen a site this login
  // → chooser. `current` is the site whose shell/nav we render; a single-site
  // user's is fixed. FS site = the "Quản lý sản phẩm" nav only.
  const { allowed, current } = await getSiteContext()
  if (current === null) redirect('/select-site')
  const isFsSite = current === 'fs'
  const canSwitchSite = allowed.size > 1
  ...
  // on the FS site (no announcements tab there).
  const announcementsUnread = profile.role !== 'admin' && !isFsSite ? await getUnreadAnnouncementCount() : 0
  ...
  const tasksPending = isStaff && !isFsSite ? await getStaffPendingTaskCount() : 0
    ...
    <ThemeProvider>
      <UserProvider profile={profile as UserProfile}>
        ...
        <div className="flex h-[100dvh] overflow-hidden">
          ...
          {/* Staff get no desktop Sidebar. On the FS site the Sidebar collapses to
              the single FS item; canSwitchSite adds a "Đổi site" control (dual users). */}
          {!isStaff && <Sidebar announcementsUnread={announcementsUnread} kpiCampaignEnabled={kpiCampaignEnabled} isFsStore={isFsSite} canSwitchSite={canSwitchSite} />}
          ...
          <main className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden bg-muted/20 pb-[calc(5.5rem_+_env(safe-area-inset-bottom))] md:pb-0">
            {/* Mobile top header */}
            <MobileHeader canSwitchSite={canSwitchSite} />
            {children}
          </main>
        </div>

        {/* Bottom navigation — mobile only */}
        <BottomNav announcementsUnread={announcementsUnread} tasksPending={tasksPending} isFsStore={isFsSite} />
        </NotificationProvider>
      </UserProvider>
    </ThemeProvider>
  )
