...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    ...
    .select('order_code, submitted_at, status, notes, stores(name), submitter:users!submitted_by(full_name), prescription_images(id)')
  ...
  const rows = (data ?? []).map((s) => ({
    ...
    'Số ảnh':       ((s.prescription_images as unknown as unknown[]) ?? []).length,
